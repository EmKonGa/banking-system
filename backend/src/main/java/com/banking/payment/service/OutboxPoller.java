...
import com.banking.common.resilience.KafkaEventPublisher;
        ...
        for (OutboxEvent event : pending) {
            try {
                ...
                if (attempts >= MAX_RETRIES) {
                    ...
                    log.error("[OUTBOX] Event id={} transaction={} PERMANENTLY FAILED after {} attempts",
                            event.getId(), event.getAggregateId(), attempts);
                } else {
                    log.warn("[OUTBOX] Event id={} transaction={} failed (attempt {}), will retry",
                            event.getId(), event.getAggregateId(), attempts);
                }
            }
